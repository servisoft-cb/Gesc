...
uses UDM1, UDM2, UPessoaProcesso;
                      
{$R *.DFM}

procedure TfPessoa.Exclui_Pessoa;
begin
...
begin
  if DM1.tUsuarioInsPessoa.AsBoolean then
    begin
      ...
      DM1.tPessoaCliente.AsBoolean := True;
      DM1.tPessoa.Post;
      DM1.tPessoa.Edit;
      Habilita;
      DBEdit1.SetFocus;
...
begin
  ...
  if DM1.tPessoaSituacao.AsString = '' then
    ShowMessage('Falta informar a situação!')
  else
  if not(DM1.tPessoaCliente.AsBoolean) and not(DM1.tPessoaOutros.AsBoolean) then
    ShowMessage('Falta escolher o tipo do registro!')
  else
  if (DM1.tPessoaSituacao.AsString = 'A') and (DM1.tPessoaDtDemissao.AsDateTime > 0) then
    ShowMessage('Cliente ativo, não pode possuir data de final de contrato!')
  else
    begin
      DM1.tPessoa.Post;
      Habilita;
...
procedure TfPessoa.SpeedButton7Click(Sender: TObject);
begin
  RxDBLookupCombo18.ClearValue;
end;

procedure TfPessoa.SpeedButton9Click(Sender: TObject);
begin
...
procedure TfPessoa.RxDBLookupCombo1Enter(Sender: TObject);
begin
  DM1.tPessoa.IndexFieldNames := 'Nome';
end;

procedure TfPessoa.RxDBGrid1DblClick(Sender: TObject);
begin
  fPessoaProcesso := TfPessoaProcesso.Create(Self);
  fPessoaProcesso.ShowModal;
end;
